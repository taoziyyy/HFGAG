...
def make_tags(tags):
    ...
    for i, tag in enumerate(tags):
        sent, start_offset, end_offset, tag_type, mention, Id = tag[0], tag[1], tag[2], tag[-1], str(tag[3]).lower(), \
                                                                tag[-2]
...
def Generate_data_rnn(genes, diseases, word_sequences, fixed_features):
    index = 0
    new_word_squences = word_sequences.copy()
    df_fixed_features = []
    df_word_sequences = []
    gdas = []
    for geneId in genes.keys():
        for diseaseId in diseases.keys():
            ...
            pubmed_features = []
            for sentence_no in range(len(new_word_squences)):  # first sentence
                sentence = new_word_squences[sentence_no]
                features = fixed_features[sentence_no]
                new_features = []
                for token_no in range(len(sentence)):  # first token of first sentence
                    feature = features[token_no]  # [1, 0, 0, 0]
                    token = sentence[token_no]  # <F000029>
            ...
            if find_target_disease and find_target_gene:
                ...
                df_word_sequences.append(new_word_squences)
                gdas.append([geneId, diseaseId])

    return df_word_sequences, df_fixed_features, gdas
def Generate_sequences_data(word_sequences_list, gdas):
    new_seq_list = []
    df = pd.DataFrame(gdas)
    for list_no in range(len(word_sequences_list)):  # first sentence
        # print(df.loc[list_no, "geneId"])
        new_seq = word_sequences_list[list_no].split(' ')
        target_gene = '<' + df.loc[list_no, "geneId"] + '>'
        target_disease = '<' + df.loc[list_no, "follicleId"] + '>'
        for token_no in range(len(new_seq)):
            token = new_seq[token_no]
            if target_disease == token:
                new_seq[token_no] = "@FOLLICLE$"
            elif target_gene == token:
                new_seq[token_no] = "@GENE$"
