...
class BertBigriCrfForNer(BertPreTrainedModel):
    def __init__(self, config):
        super(BertBigriCrfForNer, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.bigru = nn.GRU(config.hidden_size, config.hidden_size // 2, num_layers=2, bidirectional=True, batch_first=True)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        sequence_output, _ = self.bigru(sequence_output)
...
model = BertBigriCrfForNer.from_pretrained(
    "bert-base-cased",
    config=config
)
...
# 保存数据到文件
with open('save_model/bert-bigru_metrics.pkl', 'wb') as file:
    pickle.dump(result_data, file)
model.save_pretrained("save_model/bert-bigru-crf_model_ner")
