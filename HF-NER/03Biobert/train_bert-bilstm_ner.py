...
class BertBilstmForNer(BertPreTrainedModel):
    def __init__(self, config):
        super(BertBilstmForNer, self).__init__(config)
...
model = BertBilstmForNer.from_pretrained(
    "bert-base-cased",
    config=config
)
...
# 保存数据到文件
with open('save_model/bert-bilstm_metrics.pkl', 'wb') as file:
    pickle.dump(result_data, file)
model.save_pretrained("save_model/bert-bilstm_model_ner")
