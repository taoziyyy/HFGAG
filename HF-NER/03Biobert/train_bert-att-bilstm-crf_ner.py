...
class BertBilstmCrfForNer(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.attention = nn.MultiheadAttention(config.hidden_size, num_heads=4)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        weighted_output = self.attention(sequence_output, sequence_output, sequence_output)[0]
...
# 保存数据到文件
with open('save_model/bert-att-bilstm-crf_metrics.pkl', 'wb') as file:
    pickle.dump(result_data, file)
model.save_pretrained("save_model/bert-att-bilstm-crf_model_ner")
